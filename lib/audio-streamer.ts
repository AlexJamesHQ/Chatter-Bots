...
export class AudioStreamer {
  ...
  private bufferSize: number = 2400; // ~100ms chunks for rapid, fluid playback
  ...
  private initialBufferTime: number = 0.02; // 20ms initial buffer for near-zero startup delay
  ...
  addPCM16(chunk: Uint8Array) {
    ...
    // Start playing immediately if not already playing, or flush scheduled buffers.
    if (!this.isPlaying) {
      this.isPlaying = true;
